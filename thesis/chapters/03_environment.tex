...
Two environments live in the repository: the logistic game on which
every training run is executed, and a linear-regrowth fixture kept as the
test of the harvest, scarcity and absorbing-zero rules.
...
At low rates (for example \(0.2\) at \(K=40\)) rounding produces
interior stocks with zero growth, a logistic analogue of the cap-0 trap;
the rate was chosen so that none exists.
