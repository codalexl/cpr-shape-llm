...
slow-LR & naive, learning rate \(3\times 10^{-7}\), clip 0.1 & the shaper's learning rate and clip (\texttt{learning\_rate}, \texttt{cliprange}) \\
trial-batched naive & slow-LR's agent 2, updated once per trial on the concatenated batch with the GAE reset at every episode boundary & the shaper's update schedule and batch without cross-episode credit (\texttt{trial\_batched}) \\
info-off shaper & trial objective \eqref{eq:shaper-objective}, no trial prompt & cross-episode credit, the shaping term itself (\texttt{is\_shaper}; the rendered prompt is identical to the naive one, \texttt{tests/test\_cpr\_observations.py}) \\
...
The shaper differs from a naive learner in two things at once: its
advantages pass across episode boundaries inside a trial, which is the
shaping term of \eqref{eq:shaper-objective}, and it updates once per
trial from a five-episode batch instead of five times from one-episode
batches.
Under Adam the second matters on its own (the slow-LR partner takes five
times as many optimiser steps as the shaper at the same learning rate),
so a contrast of info-off against slow-LR would conflate them.
The trial-batched naive control keeps the schedule and the batch and
resets the advantage pass at every episode boundary, which is the
construction ShapeLLM's own rollout code anticipates in a comment
(\texttt{environment.py}, episode-only ids); the rung trial-batched to
info-off is then the test of the shaping term and nothing else.
The cell ``trial prompt without trial objective'' is not run; if H-B4 is
positive and H-B3 null it becomes the follow-up.
...
\begin{align}
  ...
  \label{eq:ha}\\[3pt]
  \text{H-B}k\text{:}\quad
  & \Delta^{(k)}_s(\hat{G}_{\mathrm{hawk}})>0
    \ \text{ and }\ \Delta^{(k)}_s\bigl(\hat{L}_{\mathrm{dove}}(R<12)\bigr)>0
    \quad\text{for every } s,\ k\in\{1,2,3,4\} \ \text{(Stage~B)},
  \label{eq:hb}\\[3pt]
  \text{H-X:}\quad
...
(learning rate and clip); \(k=2\): trial-batched minus slow-LR (update
schedule); \(k=3\): info-off minus trial-batched (cross-episode credit,
the shaping term); \(k=4\): naive--shaper minus info-off (trial prompt);
the overall contrasts naive--shaper minus naive--naive and minus
trial-batched are reported alongside.
The readouts are role-invariant: \(\hat{G}_{\mathrm{hawk}}\) is the
larger of the two agents' window returns and \(\hat{L}_{\mathrm{dove}}\)
the low-stock restraint of the other agent, so a rung that involves
naive--naive, where the roles are symmetric a priori and which agent
doves is a coordination outcome, compares hawk with hawk and dove with
dove whichever index took the role; joint return and equality are
reported with them, and the agent-indexed readouts alongside for the
rungs in which agent~2 is a hawk by construction.
H-X (exploitation) compares the shaper's return with the plain-hawk cell
\(72\) and reads the excess against the ceiling \(105.5\).
...
For H-X the null is a shaper return inside the interval of the plain-hawk
...
\paragraph{Amendment of 13 September (after blocks 1 and 2).}
Rule R1 passed and C2 held on every seed (Section~\ref{sec:grid-c1c2}).
Return was not stationary over epochs 81--100 on any of the six Test~A
seeds, and two seeds lost a high-return policy after epoch 80, so the
length rule R2 is applied before the ladder runs rather than after: the
Stage~B ladder and transfer arms run 200 epochs from the start, with five
seeds on the ladder, and the window is epochs 181--200.
The noise tables were generated at 200 epochs, so no draw changes.
Stage~A ladder arms keep 100 epochs, since H-A is a bound on a
difference and not a convergence claim.
This amendment is logged here and in \texttt{docs/EXPERIMENT\_PLAN.md}
before any ladder run was launched.
In the same amendment the trial-batched naive control was added as a
rung between slow-LR and info-off, and the H-B readouts were made
role-invariant, for the reasons given in Section~\ref{sec:ladder}.

\paragraph{Transfer.}
The epoch-200 adapter of each naive--shaper seed is saved; a fresh naive
learner is trained against it, frozen, for 100 epochs, with the shaper's
...
had agreed on every seed; that is why the grid runs long and five seeds
on that contrast.
The pilot observation that a slowed agent~2 stayed in the hawk role is
recorded as motivation for the timescale rung, and nothing in the design
rests on it: the rungs are justified by the keys they change, and the
role-invariant readouts make the contrasts independent of which agent
takes which role.
