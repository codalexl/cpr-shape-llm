...
\section{Executed arm --- noise on the locked logistic}

Add noise to the \textbf{same} integer update
(\(R_0=8\), \(K=40\), \(T=36\), \(\texttt{rate\_tenths}=9\)),
\texttt{noise\_tenths=5}.
Not a revival of \texttt{stochastic\_cpr\_env.py}.
Seed 0: \texttt{checkpoints/cpr\_log\_testA\_center\_noise}.
Seeds 1--2: \texttt{checkpoints/cpr\_log\_testA\_center\_noise\_s12}.
Survival \(76\), \(140\), \(141\) out of 225.
Last epoch left 2 via opening 0 on all three seeds.
Interpretation is in Chapter~\ref{ch:results}, not here.
...
Slow-LR naive--naive \(3\times 15\), matched long naive
(\texttt{naive\_naive\_center\_e50}), and the noise arm are on disk.
No further GPU jobs.

\section{Student-owned Results}

Prose that answers the opening-statistic, Test~B, seed-2, grid
success/null, two-learner, whitened extra seeds, noise, and matched
long naive is in Chapter~\ref{ch:results}.
