...
This chapter positions the thesis against the opponent-shaping and
commons literature.
What has been run is learner-versus-frozen-bot PPO and centred
two-learner PPO on a logistic chicken CPR
(\(R_0=8\), \(K=40\), \(T=36\), \(\texttt{rate\_tenths}=9\)),
first deterministic (Stage~A) then with multiplicative \(\xi\) on the
growth increment (Stage~B).
Two questions remain open after the executed ladder: whether noise as
the object of study yields a shaping result, and whether a shaper in an
LLM commons is a clear success.
...
reduction in LLM RL.
Stage~B uses paired \(\xi\) tables as a measurement device (identical
streams across arms); that is not Piche's training baseline and does not
close Gap~01.
...
\section{What this chapter leaves to Results}
...
Stage~B runs multiplicative \(\xi\) on the locked integer growth
increment; whether that is a shaping result is Chapter~\ref{ch:results}.
