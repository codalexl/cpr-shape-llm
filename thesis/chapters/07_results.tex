...
\texttt{/std}) moved the policy off opening 2 on three seeds.
Batch whitening did that on seeds 1--2 and did not on seed 0
(survival \(28/225\), last epoch \(14\times 2\)).
...
\caption{Share of episodes that do not open 2 (open 0 or 1) against a
  ...
  The whitened curve in this figure is seed~0 only, which stays on the
...
\caption{Collapse step among episodes that die, first three versus last
  ...
  The whitened panel is seed~0, which keeps dying near round~4.
...
At epoch 50 the shaper run's agent 2 still opened \(15\times 2\).
One seed each.
Not a 36-step dove policy.
...
\section{Noise arm (executed)}

Centred Test~A versus frozen always-2, \texttt{noise\_tenths=5},
15 epochs, three seeds.
Survival \(76/225\), \(140/225\), \(141/225\).
Leave-2 in the last three epochs \(80\%\), \(100\%\), \(98\%\).
Last-epoch openings: \(14\times 0\), \(15\times 0\), \(15\times 0\).
Last-epoch returns \(72.4\), \(75.2\), \(87.3\).
Last epoch left 2 via opening 0 on every seed.
Seed 0 whole-run survival is the low one.
This chapter does not write a growth-noise mechanism.
