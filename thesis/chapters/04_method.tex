...
The prior is not flattened, score scaling and score normalisation are
off, and no action-specific bonus is added: the reward is the receipt of
\eqref{eq:harvest} and the penalty of \eqref{eq:klreward} and nothing
else.
An entropy coefficient of \(0.15\) in an early pilot moved the learner
onto take-3 rather than restraint; every live configuration uses
\(0.05\), decaying to \(0.01\).
...
Every run of the final grid uses the fix.
Chapter~\ref{ch:lim} states what the shared stream does to the inference
from the earlier pilots; the seed-fixed pilots and the grid are the only
runs described as having independent seeds.
...
Section~\ref{sec:ladder} gives agent~2 the same learning rate and
