"""Decomposed cross-episode credit for shapers: the trainer adds the term to the policy advantage only, and a shaper's
...
def stub_agent(credit, shaper=True):
    ...
    agent = types.SimpleNamespace(trial_batched=False, is_shaper=shaper, cross_episode_credit=credit, cross_episode_weight=1.0,
                                  ...
                                  _record_openings=lambda *a: None, _record_live_advantages=lambda *a: None,
                                  _print_stats=lambda *a: None, logger=types.SimpleNamespace(log_stats=lambda *a: None))

    def step(queries, responses, rewards):
        term = agent.trainer.cross_episode_bonus
        seen.update(ids=list(agent.trainer.env_ids), n=agent.trainer.n, term=None if term is None else term.tolist())
        return {}
    trainer.step = step
    return agent, seen
...
def test_a_decomposed_shaper_update_passes_episode_ids_and_the_term_for_one_step():
    agent, seen = stub_agent("decomposed")
    PPOAgent.update_parameters(agent, trial())
    assert seen["ids"] == [0, 1, 0, 1, 2, 3, 2] and seen["n"] == 4
    assert seen["term"] == [3.0, -3.0, 3.0, -3.0, 0.0, 0.0, 0.0]
    assert agent.trainer.cross_episode_bonus is None and agent.n_updates == 1


def test_trial_gae_is_unchanged():
    agent, seen = stub_agent("trial_gae")
    PPOAgent.update_parameters(agent, trial())
    assert seen["ids"] == [0, 1, 0, 1, 0, 1, 0] and seen["n"] == 2 and seen["term"] is None
